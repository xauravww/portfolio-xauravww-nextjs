...
const Navbar = () => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && navbarToggleState) {
  ...
  return (
    <div
      className={`sticky nav-container bg-[#1A1D24] z-[50] relative transition-all duration-300 ${
          ...
          {[
            ...
            <li key={item.name} className="overlay-item w-full text-center opacity-0">
              <Link
                ...
                duration={500}
                onClick={() => setNavbarToggleState(false)}
                className="block py-4 text-2xl transition-colors duration-200 hover:text-[#4A90E2]"
              >
                {item.name}
              </Link>
            </li>
      ...
      <div className="fixed top-5 rounded-full left-1/2 transform -translate-x-1/2 bg-[#1A1D24]/80 backdrop-blur-md border border-[#33373E] z-[12] navbar px-8 py-2 h-[4rem] max-w-fit flex justify-center items-center shadow-lg">
        <ul className="nav-items hidden md:flex gap-4 text-white px-4 py-2 cursor-pointer text-xl md:text-2xl">
          {[
            ...
            <li key={item.name} className="li-item relative">
              <Link
                activeClass="active"
                to={item.section}
                spy={true}
